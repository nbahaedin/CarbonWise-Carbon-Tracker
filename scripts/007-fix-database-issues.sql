-- Fix the ambiguous column reference in functions
...
BEGIN
  ...
  -- Calculate streak - fix ambiguous column reference
  SELECT MAX(a.date) INTO last_activity_date 
  FROM activities a
  WHERE a.user_id = NEW.user_id AND a.date < NEW.date;
...
-- Fix the generate_weekly_report function
...
BEGIN
  ...
  SELECT 
    COALESCE(SUM(a.carbon_amount), 0),
    COUNT(*)
...
-- Add missing constraint for weekly_reports
ALTER TABLE weekly_reports 
ADD CONSTRAINT unique_user_week UNIQUE (user_id, week_start);

-- Fix activity_templates table reference
UPDATE activity_templates SET emission_factor_id = 1 WHERE emission_factor_id IS NULL;
